...
Wolf.Themes.Wolfpack = BuildTheme("Wolfpack", "#BE1923", "#BE1923", "#BE1923", "#BE1923", "#EBEBEB", "#919191", "#050507", "#0A0A0D", "#BE1923")
Wolf.Themes.Crimson = BuildTheme("Crimson", "#D6197A", "#FF5FAE", "#D6197A", "#FF5FAE", "#FFE6F2", "#DDA0C0", "#140010", "#22071B", "#FF5FAE")
Wolf.Themes.Obsidian = BuildTheme("Obsidian", "#6B6B76", "#9A9AA6", "#6B6B76", "#9A9AA6", "#F0F0F2", "#8C8C96", "#08080A", "#131316", "#B8B8C2")
Wolf.Themes.Frostbite = BuildTheme("Frostbite", "#2FCFEA", "#9FF0FF", "#2FCFEA", "#9FF0FF", "#EAFBFF", "#9FCEDD", "#020F14", "#081E26", "#9FF0FF")
Wolf.Themes.Venom = BuildTheme("Venom", "#6FE22E", "#A6FF7A", "#6FE22E", "#A6FF7A", "#E9FFE0", "#9FCB86", "#050D02", "#0D1C06", "#A6FF7A")
Wolf.Themes.Copperhead = BuildTheme("Copperhead", "#B5602A", "#E08A52", "#B5602A", "#E08A52", "#FFEEDD", "#D2A788", "#130A02", "#221405", "#E08A52")
Wolf.Themes.Glacier = BuildTheme("Glacier", "#C7E6F5", "#FFFFFF", "#C7E6F5", "#FFFFFF", "#FFFFFF", "#C6DEE8", "#060E14", "#0F1D26", "#FFFFFF")
Wolf.Themes.Nightfall = BuildTheme("Nightfall", "#4A4FD8", "#8A8FFF", "#4A4FD8", "#8A8FFF", "#E7E9FF", "#A7ADD6", "#05050F", "#0D0D22", "#8A8FFF")
Wolf.Themes.Solarflare = BuildTheme("Solarflare", "#FFA82E", "#FFD27A", "#FFA82E", "#FFD27A", "#FFF6E0", "#E0B980", "#140900", "#241300", "#FFD27A")
Wolf.Themes.Abyssal = BuildTheme("Abyssal", "#12897E", "#4FD6C9", "#12897E", "#4FD6C9", "#DFFFFC", "#7FC5C0", "#020C0B", "#061917", "#4FD6C9")
Wolf.Themes.Rustic = BuildTheme("Rustic", "#7A4A2E", "#B07850", "#7A4A2E", "#B07850", "#F2E3D5", "#C7A889", "#0E0803", "#1B1006", "#B07850")
Wolf.Themes.Jadeite = BuildTheme("Jadeite", "#1FA66B", "#5FE0A6", "#1FA66B", "#5FE0A6", "#E4FFEF", "#91CBA8", "#020F08", "#082014", "#5FE0A6")
Wolf.Themes.Stormfront = BuildTheme("Stormfront", "#5A6B7D", "#8FA0B0", "#5A6B7D", "#8FA0B0", "#EEF2F6", "#A3ADB6", "#0A0D10", "#161B20", "#8FA0B0")
Wolf.Themes.Moonlit = BuildTheme("Moonlit", "#B8AEEA", "#E4DEFF", "#B8AEEA", "#E4DEFF", "#F5F5FF", "#B5B5D9", "#0A0A14", "#15151F", "#E4DEFF")
Wolf.Themes.Wildfire = BuildTheme("Wildfire", "#FF6A1A", "#FFA05C", "#FF6A1A", "#FFA05C", "#FFEBDD", "#E0A888", "#150600", "#271000", "#FFA05C")
Wolf.Themes.Cobalt = BuildTheme("Cobalt", "#2B6FD1", "#6EA8F5", "#2B6FD1", "#6EA8F5", "#E4EEFF", "#9CB8DB", "#020914", "#071528", "#6EA8F5")
Wolf.Themes.Amberglow = BuildTheme("Amberglow", "#D9A857", "#F0CE8F", "#D9A857", "#F0CE8F", "#FFF6E4", "#E3CBA0", "#120D02", "#221A06", "#F0CE8F")
Wolf.Themes.Amethyst = BuildTheme("Amethyst", "#9B3DE0", "#C97DFF", "#9B3DE0", "#C97DFF", "#F3E9FF", "#C5AEDD", "#0C0416", "#180A2A", "#C97DFF")

Wolf.ThemeOrder = {"67", "Wolfpack", "Crimson", "Obsidian", "Frostbite", "Venom", "Copperhead", "Glacier", "Nightfall", "Solarflare", "Abyssal", "Rustic", "Jadeite", "Stormfront", "Moonlit", "Wildfire", "Cobalt", "Amberglow", "Amethyst"}
	...
	if buttons and #buttons > 0 then
		...
		for _, b in ipairs(buttons) do
			...
			nb.MouseButton1Click:Connect(function()
				...
				if b.CloseOnClick ~= false and frame.Parent then frame:Destroy() end
			end)
		end
	end

	table.insert(ActiveNotifications, { Frame = frame, Stroke = stroke, Icon = icon, Title = tLbl, Content = cLbl, Bar = bar })

	Tween(frame, 0.3, { Size = UDim2.new(1, 0, 0, (buttons and #buttons > 0) and 94 or 68) })

	if not (buttons and #buttons > 0) then
		task.delay(duration, function()
			if not frame.Parent then return end
			local out = Tween(frame, 0.2, { Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1 })
			out.Completed:Connect(function()
				for i, item in ipairs(ActiveNotifications) do
					if item.Frame == frame then table.remove(ActiveNotifications, i) break end
				end
				frame:Destroy()
			end)
		end)
...
function Wolf:CreateWindow(config)
	...
	local openButtonIcon = config.OpenButtonIcon or "https://cdn.discordapp.com/attachments/1521705447781044287/1549975636997578772/1789613245815.png?ex=6aaca6cd&is=6aab554d&hm=75f67c26ff1339f45194b03a804c409b3792c3e18b70fff35cf378632b2c4e91&"
